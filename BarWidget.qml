...
BarWidget {
  ...
  readonly property bool animations: setting("animations", true) === true
  ...
  PopupCard {
    ...
    Column {
      ...
      Toggle {
        width: parent.width
        label: "Animations"
        description: "Enable size animations."
        checked: root.animations
        foreground: root.bar ? root.bar.foreground : Color.foreground
        accent: Color.accent
        fontFamily: root.bar ? root.bar.fontFamily : Style.font.family
        onClicked: root.saveSetting("animations", !root.animations)
      }
    }
  }
